...
pub fn prompt_for_action(
    ...
    let action = select_action(&env_var_name, &comment_syntax)?;
...
fn print_finding_line(line_number: usize, content: &str) {
    println!(
        "  {} {} {}",
        colors::primary().apply_to(format!("{:>LINE_NUMBER_WIDTH$}", line_number)),
        colors::muted().apply_to("│"),
        content,
    );
}
...
fn print_context_line(line_number: usize, content: &str) {
    println!(
        "  {} {} {}",
        colors::muted().apply_to(format!("{:>LINE_NUMBER_WIDTH$}", line_number)),
        colors::muted().apply_to("│"),
...
fn select_action(
    ...
    if let Some(syntax) = comment_syntax {
        items.push(format!("Ignore       → {}", syntax.format_ignore()));
    }
