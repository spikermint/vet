...
pub fn actions_for_diagnostics(
    ...
    let Some(syntax) = comment_syntax::for_language(&document.language_id) else {
        return Vec::new();
    };
    ...
    for diagnostic in &vet_diagnostics {
        if let Some(action) = make_ignore_action(uri, diagnostic, &syntax, &document.content) {
            actions.push(CodeActionOrCommand::CodeAction(action));
        }
    }

    // "Ignore all on this line" if multiple diagnostics on same line
...
fn make_ignore_action(uri: &Url, diagnostic: &Diagnostic, syntax: &CommentSyntax, content: &str) -> Option<CodeAction> {
    ...
    let ignore_text = format!(" {}", syntax.format_ignore());
...
mod tests {
    ...
    #[test]
    fn no_actions_for_unsupported_language() {
    ...
    fn edit_position_is_at_line_end() {
        ...
        if let CodeActionOrCommand::CodeAction(action) = &actions[0] {
            ...
            assert_eq!(text_edits[0].range.start.character, content.len() as u32);
        } else {
            panic!("Expected CodeAction");
        }
    }
    ...
    fn multiple_diagnostics_same_line_produces_bulk_action() {
        ...
        // Should have 2 individual actions + 1 bulk action
