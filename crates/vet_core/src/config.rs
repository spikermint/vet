...
mod tests {
    ...
    #[test]
    fn from_toml_parses_include_low_confidence_flag() {
        let toml = "include_low_confidence = true";
        let config = Config::from_toml(toml).unwrap();
        assert!(config.include_low_confidence);
    }
    ...
    fn custom_pattern_compile_assigns_custom_group() {
        ...
        let compiled = pattern.compile().unwrap();
        assert_eq!(compiled.group.as_ref(), "custom");
    }
    ...
    fn config_survives_serialise_deserialise_roundtrip() {
        ...
        assert_eq!(restored.include_low_confidence, original.include_low_confidence);
